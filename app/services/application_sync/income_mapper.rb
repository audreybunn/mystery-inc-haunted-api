  ...
  module ApplicationSync
    class IncomeMapper
      ...
      def map_plaid_income
        ...
        # FIX: gross/net were previously swapped, corrupting mapped income data.
        {
      ...
      def normalize_income_type(external_type)
        ...
        # FIX: guard against nil to avoid NoMethodError on #downcase.
      ...
      def validate_required_fields!(data, required_fields)
        # FIX: account for keys that are present but blank/nil, which would
        # otherwise pass validation and produce corrupt mapped records.
        missing_fields = required_fields.reject do |field|
          data.key?(field) && !data[field].to_s.strip.empty?
        end
